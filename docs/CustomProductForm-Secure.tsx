// SECURE VERSION - CustomProductForm.tsx
// Improvements:
...
export default function CustomProductForm(props: CustomProductFormProps) {
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            // SECURITY: Check file size (2MB limit to prevent base64 bloat)
            ...
            // SECURITY: Validate it's actually an image by checking magic bytes
            ...
            reader.onload = (event) => {
                ...
                // JPEG magic bytes: ffd8ffe0 or ffd8ffe1 or ffd8ffe2
                ...
                if (!isPNG && !isJPEG) {
                    setMessage({
                        type: "error",
                        text: "Invalid image file. File may be corrupted or renamed.",
                    })
                    e.target.value = ""
                    return
                }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // SECURITY: Limit number of colors
        if (formData.colors.length > 10) {
            setMessage({ type: "error", text: "Maximum 10 colors allowed" })
            setIsSubmitting(false)
            return
        }
    ...
    return (
        <div style={{ width: "100%", maxWidth: 600, margin: "0 auto", padding: 24, fontFamily: "DM Sans, system-ui, sans-serif", backgroundColor: "rgb(247, 245, 242)", borderRadius: 12, border: "1px solid rgba(0,0,0,0.08)" }}>
            ...
            <form onSubmit={handleSubmit}>
                {/* SECURITY: Honeypot field (hidden from real users) */}
                <input
                    type="text"
                    name="website"
                    value={honeypot}
                ...
                <div style={{ marginBottom: 20 }}>
                    ...
                    <input
                        ...
                        minLength={2}
                        maxLength={100}
                        style={{ width: "100%", padding: "10px 12px", fontSize: 14, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, backgroundColor: "white" }}
                    />
                </div>
                ...
                {/* Color section */}
                <div style={{ marginBottom: 20 }}>
                    <label style={{ display: "block", fontSize: 13, fontWeight: 600, marginBottom: 6, color: "#2D2D2D" }}>Colors * (Max 10)</label>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center", padding: 12, backgroundColor: "white", borderRadius: 8, border: "1px solid rgba(0,0,0,0.08)" }}>
                        ...
                        <button type="button" onClick={handleAddCustomColor} disabled={formData.colors.length >= 10} style={{ padding: "6px 12px", fontSize: 12, fontWeight: 600, backgroundColor: formData.colors.length >= 10 ? "#ccc" : "#2D2D2D", color: "white", border: "none", borderRadius: 6, cursor: formData.colors.length >= 10 ? "not-allowed" : "pointer" }}>Add Color</button>
                    </div>

                    {/* Selected colors */}
                    {formData.colors.length > 0 && (
                        <div style={{ marginTop: 12 }}>
                            <p style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Selected ({formData.colors.length}/10):</p>
                ...
                {showDesignUpload && (
                    <div style={{ marginBottom: 20 }}>
                        ...
                        <p style={{ fontSize: 11, marginTop: 6, opacity: 0.6 }}>Max 2MB. Only PNG and JPG files allowed.</p>
                    </div>
                )}
